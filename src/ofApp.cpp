...
void ofApp::setup(){
	...
	shotSound.setMultiPlay(true);
	//swingSound.setMultiPlay(true);

	//--> 調整
	gui.setup();
	gui.add( offset.setup("offset", 1.5, 0, 10.));
	gui.add( gravity.setup("gravity", gravity = -9.807*50, 0, -9.807*50) );
	...
	//--> シャトルの初期位置
	initPos = ofVec3f(.2f-offset, .8f, 0);
	shuttlePos = initPos;
	...
	*/
	...
	while(oscMocapReciever.hasWaitingMessages()){
		...
		for (int i = 0; i < TEMPLATE_RACKET_MARKER_NUM; i++) {
			...
			(float)racketMarkerPos[0][i].x -= offset;
		...
		for (int i = 0; i < TEMPLATE_RACKET_MARKER_NUM; i++) {
			...
			(float)racketMarkerPos[1][i].x += offset;
		...
		for (int i = 0; i < 2; i++) {
			...
			(float)playerMarkerPos[0][i].x -= offset;
		...
		for (int i = 0; i < 2; i++) {
			...
			(float)playerMarkerPos[1][i].x += offset;
	...
	for( int i = 0; i < TEMPLATE_RACKET_NUM; i++) {
		...
		if (bpos[i].size() > 10)
		{
			...
			if ( 0.13 > sqrt( (racketPos[i].x-shuttlePos.x)*(racketPos[i].x-shuttlePos.x) + (racketPos[i].y- shuttlePos.y)*(racketPos[i].y- shuttlePos.y) + (racketPos[i].z- shuttlePos.z)*(racketPos[i].z- shuttlePos.z) ) ) 
			{
	...
	//{
	//	//--> 初速度
	//	vector<ofVec3f>::iterator fitr = bspos.begin();
	//	vector<ofVec3f>::iterator eitr = bspos.begin() + 4;
	//	initVel = *eitr - *fitr;
	//	initVel *= 12; //速度
...
void ofApp::draw(){
	...
	//--> ラケットのマウス、判定マーカー描画
	racketPos[0].x += offset;
	racketPos[1].x -= offset;
